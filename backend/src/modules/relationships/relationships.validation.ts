    ...
    try {
      ...
      if (type === 'parent') {
        if (p1.birthDate && p2.birthDate) {
          ...
          if (parentBirth >= childBirth) throw new AppError('Parent must be born before the child', 400);
      ...
      const checkEquivalence = (otherType: string, otherFrom: string, otherTo: string) => {
        // Direct
        if (otherType === type && otherFrom === fromId && otherTo === toId) return true;
        // Inverse Hierarchical
        if (type === 'parent' && otherType === 'child' && otherFrom === toId && otherTo === fromId) return true;
        if (type === 'child' && otherType === 'parent' && otherFrom === toId && otherTo === fromId) return true;
        if (type === 'parent' && otherType === 'adopted_child' && otherFrom === toId && otherTo === fromId) return true;
        if (type === 'adopted_child' && otherType === 'parent' && otherFrom === toId && otherTo === fromId) return true;
        // Bidirectional
        if (isBidirectional && otherType === type && otherFrom === toId && otherTo === fromId) return true;
        return false;
      };
      ...
      for (const record of pendingResult.records) {
        ...
        if (excludeProposalId && rp.id === excludeProposalId) continue;

        if (checkEquivalence(rp.relationshipType, rp.fromPersonId, rp.toPersonId)) {
          throw new AppError(`A ${rp.relationshipType} proposal [ID: ${rp.id}] is already pending between these people`, 400);
        }
      }
      ...
      for (const record of officialResult.records) {
        ...
        if (checkEquivalence(r.type, startId, endId)) {
          throw new AppError(`This relationship already exists in the tree`, 400);
        }
      }
